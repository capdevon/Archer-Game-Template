...
import com.capdevon.control.CameraHandler;
...
/**
 */
public class Test_Swimming extends BaseGameApplication implements ActionListener {
...
//        System.out.println(JmeSystem.getStorageFolder().toString());
    ...
    public void simpleInitApp() {
        ...
        ginput.addActionListener(this);
        stateManager.attach(ginput);
    }
    
    private void setLogDebug(boolean enable) {
//    	Logger.getLogger("").setLevel(Level.FINE);
    	Logger rootLogger = LogManager.getLogManager().getLogger("");
    	for (Handler h : rootLogger.getHandlers()) {
    		if (h instanceof ConsoleHandler) {
    			System.out.println(h);
    			((ConsoleHandler) h).setLevel(Level.FINE);
//    			((ConsoleHandler) h).setFormatter(new JmeFormatter());
    		}
    	}
    }
    
    private void setupVideoImageAppStates() {
        String dirName = System.getProperty("user.dir") + "/video-images/";
        System.out.println(dirName);
        
        ScreenshotAppState screenshot = new ScreenshotAppState(dirName, "image-", 0); //Stamp
        stateManager.attach(screenshot);
        
        String fileName = String.format("%s-%d.avi", settings.getTitle(), System.currentTimeMillis() / 1000);
        VideoRecorderAppState recorder = new VideoRecorderAppState(0.5f, settings.getFrameRate());
        recorder.setFile(new File(dirName, fileName));
    ...
    public void setupScene() {
        ...
        for (Spatial sp : scene.getChildren()) {
            ...
            if (sp.getName().equals("Pool")) {
                ...
                System.out.println(sp.getLocalTranslation());
        ...
        level.attachChild(SkyFactory.createSky(
                assetManager, "Scenes/Beach/FullskiesSunset0068.dds", SkyFactory.EnvMapType.CubeMap));
...
//        if (name.equals(KeyMapping.TOGGLE_PHYSICS_DEBUG) && isPressed) {
//            boolean debug = getPhysics().isDebugEnabled();
//            getPhysics().setDebugEnabled(!debug);
//        }
    }
    ...
    private class PlayerControl extends AdapterControl implements ActionListener, TriggerListener {
        ...
        public Transform getTransform() {
        	return transform.set(spatial.getWorldTransform());
        }
